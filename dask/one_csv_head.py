...
all_df = pd.concat([df2018.head(),df2017.head()], axis=0, ignore_index=True, sort=False)
del [[df2017,df2018]]
gc.collect()
...
# df2016.head(10)
all_df = pd.concat([all_df,df2016.head()], axis=0, ignore_index=True, sort=False)
del [[df2016]]
gc.collect()
print ('Execution Time: ', (time.time()- start)/60 ,' minutes')
print("step 3/4!")
...
all_df = pd.concat([all_df,df2015.head()], axis=0, ignore_index=True, sort=False)
del [[df2015]]
gc.collect()
print ('Execution Time: ', (time.time()- start)/60 ,' minutes')
print("step 4/4!")
...
all_df = pd.concat([all_df,df2014.head()], axis=0, ignore_index=True, sort=False)
# del [[all_df]]
# gc.collect()
print ('Execution Time: ', (time.time()- start)/60 ,' minutes')
...
all_df.to_csv('./csv/all_years_proper_head.csv', mode='a', header=False) # To CSV
...
all_df.to_sql(con=engine, name='all_years_proper_template', if_exists='append') # Code To Save Into Mysql/Mariadb Database
